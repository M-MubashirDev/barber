...
//   const inquiries = timeData?.inquiries || []; // Adjusted based on your data structure
//   console.log(inquiries);
//   // Filter inquiries to only those matching the current professional ID
//   const filteredInquiries = useMemo(
//     () => inquiries.filter((inq) => inq.professional === id),
//     [inquiries, id]
//   );
//   console.log(filteredInquiries, "sad");
...
function Time() {
  ...
  useEffect(() => {
    if (timeError) {
      console.error("Error fetching inquire data:", timeError.message);
      // Optionally show a notification or navigate to error page
    }
    if (shopTimeError) {
      console.error("Error fetching shop timing data:", shopTimeError.message);
      // Optionally show a notification or navigate to error page
  ...
  // Destructure professional info (if any)
  if (!professionalData) {
    // If the ID is invalid or sessionStorage has no matching pro
    return <div>Professional not found or not in session storage.</div>;
  }
  const { image = "imgCircle.png", name, notAvailable, _id } = professionalData;

  // 1) Safely extract inquiries as an array
  const inquiries = Array.isArray(timeData?.inquiries)
    ? timeData.inquiries
    : [];

  // 2) Filter inquiries to only those matching the current professional ID
  const filteredInquiries = useMemo(() => {
    return inquiries.filter((inq) => {
      // If `inq.professional` is an Object, check `._id`
      // If `inq.professional` is a string, compare directly
      return inq.professional === id || inq?.professional?._id === id;
    });
  }, [inquiries, id]);

  // 3) Build blockedSlots from inquiries
  const blockedSlotsFromInquiries = useMemo(
    () => buildBlockedSlotsFromInquiries(filteredInquiries),
    [filteredInquiries]
  );

  // 4) Build blockedSlots from notAvailable
  const blockedSlotsFromNotAvailable = useMemo(
    () => buildBlockedSlotsFromNotAvailable(notAvailable),
    [notAvailable]
  );

  // 5) Combine both blockedSlots
  const blockedSlots = useMemo(() => {
    return [...blockedSlotsFromInquiries, ...blockedSlotsFromNotAvailable];
  }, [blockedSlotsFromInquiries, blockedSlotsFromNotAvailable]);
